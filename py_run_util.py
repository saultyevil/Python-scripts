...
def process_line_output(line:str, spec_cycle:bool, not_quiet:bool=True, show_all:bool=False) -> bool:
    """
    ...
    line: str
        The line to process
    spec_cycle: bool
        If True, then process a line from a spectral cycle
    not_quiet: bool, optional
        If False, no output will be shown unless show_all is True
    show_all: bool, optional
        If True, every line will be printed to screen

    Returns
    -------
    spec_cycle: bool
        If True, the currently processes line was part of a spectral cycle
    """
...
def find_number_of_physical_cores_lscpu() -> float:
    """
    Find the physical number of CPU cores in a computer.
...
def get_num_procs(default_cores:int=0) -> Tuple[bool, int]:
    """
    Determine the number of Python processes to run.

    Parameters
    ----------
    default_cores: int, optional
        It provided, the function will assume that this is the number of cores Python will be run using and that the
        computer is capable of using this many cores

    Returns
    -------
    mpi: bool
        If True, MPI can be used to parallelise Python
    n_cores: int
...
def check_convergence(root_name:str, work_dir:str) -> Union[int, float]:
    ...
    converge_fraction: int or float
        If an int is return, there has been an error in finding the convergence fraction of the simulation. Otherwise,

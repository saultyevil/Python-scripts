...
def plot_python_wind(root_name: str, output_name: str, path: str = "./", vars: List[str] = None,
                     var_type: List[str] = None, subplot_dims: Tuple[int, int] = None,
    ...
    if data_ndims != "2d":
        print("py_plot.plot_python_wind: only understand ndims = 2d")
        return
    ...
    for i in range(subplot_dims[0]):
        for j in range(subplot_dims[1]):
            ...
            x, z, qoi = py_plot_util.get_wind_data(root_name, var, var_t, path)
            ...
            if len(qoi) == 1 and len(x) == 1 and len(z) == 1:
                index += 1
                continue
            # This is to avoid division by 0 errors which """don't""" matter :^)
...
def plot_spec_comps(spec_patch: str, output_name: str, loglog_scale: bool = False, smooth: int = 15,
    ...
    for i in range(len(headers_top)):
        ...
        flux = py_plot_util.smooth_1d_array(np.array(spec[1:, spec[0, :] == headers_top[i]], dtype=float), smooth, verbose)
    ...
    for i in range(len(headers_bot)):
        ...
        flux = py_plot_util.smooth_1d_array(np.array(spec[1:, spec[0, :] == headers_bot[i]], dtype=float), smooth, verbose)
        if loglog_scale:
...
def main() -> None:
    ...
    if len(spec_files) == 1:
        ...
        plot_python_wind(root, outname + "_wind", path, vars, var_types, filetype=FILETYPE, verbose=VERBOSE)
        # Now plot some ions
        print("\nPlotting wind ions")
        vars = ["H_i01", "H_i02", "C_i03", "C_i04", "C_i05", "Si_i04", "N_i05", "O_i06"]
        var_types = ["ion"] * len(vars)
        plot_python_wind(root, outname + "_ions", path, vars, var_types, filetype=FILETYPE, verbose=VERBOSE)
        print("")  # spacer :-)

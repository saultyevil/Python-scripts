...
def plot_line_id(ax: plt.Axes, ymax) -> plt.Axes:
    """
    ...
    for i in range(nlines):
        ...
        ax.text(x, ymax + 0.2, lab, ha="center", va="center", rotation="vertical")
...
def plot_uv_observations():
    ...
    composite_qso = ppu.read_spec_file("/home/saultyevil/PySims/tde/observed_spec/sdss_composite_qso.dat").astype(float)

    # Book keeping lists for plotting the spectra in a loop
    nspec = 4
    spec_list = [composite_qso, asassn14li, iptf15af, iptf16fnl]
    spec_names = ["SDSS Composite QSO", "ASASSN14li", "iPTF15af",  "iPTF16fnl", "AT2018zr"]
    ...
    ax = plot_line_id(ax, i)

    fig.tight_layout()
    plt.savefig("tde_uv_observations.pdf")
    plt.show()
